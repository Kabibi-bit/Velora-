...
function renderNav(activePage){
  ...
  const logoHref = role === 'candidate' ? 'overview.html' : 'index.html';
...
/* ---- Waypoint: a social space ranked by roadmap relevance, not
   engagement. The differentiator vs a generic feed: every post and
   suggested connection is ranked by real overlap with YOUR stated
   goal and roadmap, and that reasoning is shown, not hidden. Mirrors
   app/services/social.py exactly, so this demo behaves the same way
   the real backend will once connected. Peers here are mock personas
   (consistent with MOCK_CANDIDATES/MOCK_REQUESTS elsewhere) since a
   real feed needs real other users, which a single-account demo
   doesn't have. ---- */
const WAYPOINT_PEERS = [
  {id:'p1', northstar: 'Break into data-focused product management', skills: 'SQL, Python, A/B testing, product analytics', stage: 'student'},
  {id:'p2', northstar: 'Land a machine learning engineering role', skills: 'Python, PyTorch, SQL, ML pipelines', stage: 'grad'},
  {id:'p3', northstar: 'Transition into UX research from a design background', skills: 'Figma, user interviews, synthesis', stage: 'switch'},
  {id:'p4', northstar: 'Break into product management from a growth marketing background', skills: 'A/B testing, SQL, growth experimentation', stage: 'switch'},
  {id:'p5', northstar: 'Become a data analyst supporting product decisions', skills: 'SQL, Python, dashboards, product analytics', stage: 'student'},
  {id:'p6', northstar: 'Grow into a senior product manager role', skills: 'Roadmap planning, stakeholder communication, SQL', stage: 'grad'},
];

const WAYPOINT_POSTS = [
  {id:'w1', peer_id:'p1', body: 'Finally shipped the SQL + Python analytics project I\'ve been stuck on for weeks - turns out the blocker was overcomplicating the join logic, not the actual analysis.', video_url: null, roadmap_stage: 2, roadmap_stage_title: 'Build a portfolio project', created_at: daysAgoISO(1)},
  {id:'w2', peer_id:'p3', body: 'Did my first 3 informational interviews this week translating my design background into UX research language. Way less scary than I expected once I had a specific pitch.', video_url: null, roadmap_stage: 1, roadmap_stage_title: 'Write a pitch translating your background', created_at: daysAgoISO(2)},
  {id:'w2v', peer_id:'p2', body: 'Recorded a quick walkthrough of the ML eval project I shipped this week - precision/recall tradeoffs finally clicked once I visualized them.', video_url: 'https://example.com/demo-video-link', roadmap_stage: 3, roadmap_stage_title: 'Build a public record scouts can verify', created_at: daysAgoISO(3)},
  {id:'w3', peer_id:'p4', body: 'Got real feedback on my A/B testing case study from a growth PM today - turns out I was leading with the wrong metric the whole time. Redoing it this weekend.', video_url: null, roadmap_stage: 2, roadmap_stage_title: 'Apply to entry-level roles matching your goal', created_at: daysAgoISO(4)},
  {id:'w4', peer_id:'p5', body: 'Two rejections this week, both with real feedback attached instead of silence. Genuinely more useful than an offer would have been at this stage.', video_url: null, roadmap_stage: 3, roadmap_stage_title: 'Apply to your top matches', created_at: daysAgoISO(5)},
  {id:'w5', peer_id:'p6', body: 'Realized I\'d been avoiding asking my manager directly what "senior" actually means on my team. Asked this week - turns out it\'s a lot more concrete than I assumed.', video_url: null, roadmap_stage: 1, roadmap_stage_title: 'Build one documented win', created_at: daysAgoISO(6)},
];

function daysAgoISO(days){ return new Date(Date.now() - days * 86400000).toISOString(); }

function getWaypointPostsAll(){
  try{ return [...(JSON.parse(localStorage.getItem('velora_waypoint_own_posts')) || []), ...WAYPOINT_POSTS]; }
  catch(e){ return WAYPOINT_POSTS; }
}
function saveOwnWaypointPost(post){
  const own = JSON.parse(localStorage.getItem('velora_waypoint_own_posts') || '[]');
  own.unshift(post);
  localStorage.setItem('velora_waypoint_own_posts', JSON.stringify(own));
}
function getWaypointConnections(){ try{ return JSON.parse(localStorage.getItem('velora_waypoint_connections')) || {}; }catch(e){ return {}; } }
function saveWaypointConnections(c){ localStorage.setItem('velora_waypoint_connections', JSON.stringify(c)); }
function getWaypointComments(postId){ try{ return (JSON.parse(localStorage.getItem('velora_waypoint_comments')) || {})[postId] || []; }catch(e){ return []; } }
function addWaypointComment(postId, body){
  const all = JSON.parse(localStorage.getItem('velora_waypoint_comments') || '{}');
  if(!all[postId]) all[postId] = [];
  all[postId].push({ body, created_at: new Date().toISOString() });
  localStorage.setItem('velora_waypoint_comments', JSON.stringify(all));
}

function computePostRelevanceHeuristic(viewerProfile, postBody, authorProfile){
  const viewerTokens = new Set([...tokenize(viewerProfile.northstar || ''), ...tokenize(viewerProfile.skills || '')]);
  const postTokens = new Set(tokenize(postBody));
  const authorTokens = new Set(tokenize(authorProfile.northstar || ''));
  const matchedTopic = [...viewerTokens].filter(t => postTokens.has(t));
  const matchedGoal = [...viewerTokens].filter(t => authorTokens.has(t));
  return {
    score: matchedTopic.length * 3 + matchedGoal.length * 2,
    matchedTopicTerms: matchedTopic.slice(0, 4),
    sameGoalDirection: matchedGoal.length > 0,
  };
}
function computeConnectionRelevanceHeuristic(viewerProfile, targetProfile){
  const viewerTokens = new Set([...tokenize(viewerProfile.northstar || ''), ...tokenize(viewerProfile.skills || '')]);
  const targetTokens = new Set([...tokenize(targetProfile.northstar || ''), ...tokenize(targetProfile.skills || '')]);
  const overlap = [...viewerTokens].filter(t => targetTokens.has(t));
  return {
    score: overlap.length * 2,
    matchedTerms: overlap.slice(0, 5),
    stageRelation: (viewerProfile.stage && targetProfile.stage) ? (viewerProfile.stage === targetProfile.stage ? 'similar_stage' : 'different_stage') : null,
  };
}

async function explainPostRelevanceDeep(viewerProfile, roadmapSummary, postBody, authorProfile){
  const prompt = `A person's goal: "${viewerProfile.northstar}". Their roadmap strategy: "${roadmapSummary || 'no roadmap yet'}".\n\nA post from someone else, whose stated goal is "${authorProfile.northstar}":\n"${postBody}"\n\nIn 2-3 sentences, explain honestly whether and why this post is actually relevant to the viewer's specific goal and roadmap - reference their real roadmap stage or goal by name. If it's not genuinely relevant, say so plainly rather than forcing a connection.`;
  try{
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST", headers: { "Content-Type": "application/json" },
    ...
    const data = await response.json();
    return (data.content || []).map(b => b.type==='text'?b.text:'').filter(Boolean).join('\n') || null;
  } catch(err){ console.error('Post relevance explanation failed:', err); return null; }
}
async function explainConnectionRelevanceDeep(viewerProfile, targetProfile){
  const prompt = `Person A's goal: "${viewerProfile.northstar}". Skills: "${viewerProfile.skills}". Stage: ${viewerProfile.stage || 'unspecified'}.\nPerson B's goal: "${targetProfile.northstar}". Skills: "${targetProfile.skills}". Stage: ${targetProfile.stage || 'unspecified'}.\n\nIn 2-3 sentences, give Person A an honest, specific reason (or lack of one) to connect with Person B - reference their actual stated goals and skills. If the overlap is weak, say so rather than manufacturing a reason.`;
  try{
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 200, messages: [{role:"user", content: prompt}] })
    ...
    const data = await response.json();
    return (data.content || []).map(b => b.type==='text'?b.text:'').filter(Boolean).join('\n') || null;
  } catch(err){ console.error('Connection relevance explanation failed:', err); return null; }
}
