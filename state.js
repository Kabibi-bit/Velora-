...
/* ---- Waypoint: a private progress journal, not a social feed.
   ...
   real progress against real roadmap stages, with an honest AI
   reflection on each entry. ---- */
function getWaypointPostsAll(){
  try{ return JSON.parse(localStorage.getItem('velora_waypoint_own_posts')) || []; }
  catch(e){ return []; }
}
...
async function reflectOnJournalEntry(profile, roadmapSummary, entryBody, roadmapStageTitle){
  const stageLine = roadmapStageTitle ? `They tagged this entry to their roadmap stage: "${roadmapStageTitle}".\n` : '';
  const prompt = `A person's goal: "${profile.northstar}". Their roadmap strategy: "${roadmapSummary || 'no roadmap yet'}".\n${stageLine}\nA journal entry they just wrote about their progress:\n"${entryBody}"\n\nIn 2-3 sentences, give an honest, specific reflection - does this genuinely represent progress toward their stated goal, is there a real risk or blind spot worth naming, or a concrete next step implied by what they wrote? Reference their actual goal or roadmap stage by name. Avoid generic encouragement like "great job" or "keep it up" - be specific or say nothing complimentary at all.`;
  try{
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST", headers: { "Content-Type": "application/json" },
