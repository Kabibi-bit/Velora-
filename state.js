...
const FACTOR_NAMES_JS = ['goalFit', 'skillFit', 'priorityFit', 'locationFit', 'deadlineUrgency', 'descriptionFit', 'semanticFit'];
...
function computeFactorInteractions(applicationsWithOutcomes){
  ...
  INTERACTION_PAIRS_JS.forEach(([label, factorA, factorB]) => {
    ...
    if(bothHigh.length < 3 || aOnly.length < 2 || bOnly.length < 2) return;
...
async function generateDeepPersonalizationInsightsJS(){
  ...
  if(apps.length < 4){
    return { insights: [], sampleSize: apps.length, note: "Not enough applications with both a draft and a logged outcome yet - need at least 4 to find a real pattern in what you've actually written, rather than guessing." };
  }
...
function scoreListing(listing, goalTokens, skillTokens, profile, factorWeights){
  ...
  const score = goalFit + skillFit + priorityFit + locationValue + deadlineValue + descriptionValue;
  // Headroom tied to actual earned contribution, not to whether title
  // text merely exists (title is basically always present, so that
  // would unfairly dilute every listing where it happened not to add
  // new signal beyond the tags for this specific profile).
  const descriptionHeadroom = descriptionValue > 0 ? 1.5 : 0;
  const denom = tagSet.length*3 + 2 + 1.5 + descriptionHeadroom;
  const pct = Math.max(35, Math.min(97, Math.round((score / denom) * 100)));

  const factorsEngaged = [goalFit, skillFit, priorityFit, locationValue, deadlineValue, descriptionValue].filter(v => v > 0).length;
  ...
  return {
    ...
    factors: { goalFit, skillFit, priorityFit, locationFit: locationValue, locationReason: location.reason, deadlineUrgency: deadlineValue, daysLeft: deadline.daysLeft, descriptionFit: descriptionValue, descriptionTerms: titleOverlap.terms },
  };
}
...
function runMatchCycle(profile){
  ...
  let scored = candidates.map(l=>{
    const m = scoreListing(l, goalTokens, skillTokens, profile, factorWeights);
    if(!m) return null;
...
function runMatchCycleWithNearMisses(profile){
  ...
  const presentable = scored.filter(l => l.pct >= PRESENTABLE_MIN_SCORE && PRESENTABLE_MIN_SIGNAL.has(l.signalStrength));
  const matches = presentable.slice(0, 10);
  const shownIds = new Set(matches.map(m => m.id));
  const nearMisses = scored.filter(l => !shownIds.has(l.id)).slice(0, 5);
