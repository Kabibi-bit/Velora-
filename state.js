...
function scoreListing(listing, goalTokens, skillTokens, profile){
  const dealbreakers = (profile.dealbreakers || '').toLowerCase();
  ...
  const score = goalFit + skillFit + priorityFit + location.value + deadline.value;
  ...
  return {
    ...
    factors: { goalFit, skillFit, priorityFit, locationFit: location.value, locationReason: location.reason, deadlineUrgency: deadline.value, daysLeft: deadline.daysLeft },
  };
}
...
function runMatchCycle(profile){
  ...
  let scored = candidates.map(l=>{
    const m = scoreListing(l, goalTokens, skillTokens, profile);
    if(!m) return null;
