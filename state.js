...
function findFabricatedNumbers(original, polished){
  ...
  const polishedNumbers = new Set(polished.match(numRe) || []);
  return [...polishedNumbers].filter(n => !originalNumbers.has(n)).sort();
}
...
async function generateResumeSummary(profile, entries){
  if(!profile.northstar && entries.length === 0) return '';
...
async function generateResume(profile){
  ...
  const summaryLine = await generateResumeSummary(profile, entries);
  ...
  const anyFlags = polishedEntries.some(pe => pe.flagged_numbers.length > 0);
  return { ...doc, review_note: anyFlags ? "One or more bullets include a number that wasn't in what you originally wrote - double check those before using this." : null };
}
...
function meaningfulTokens(text){
  return new Set(tokenizeForMatching(text).filter(t => t.length > 3 && !RESUME_STOPWORDS.has(t)));
}
...
const GENERIC_PREFIX_EXCLUSIONS_JS = new Set(['people','team','work','help','time','life','good','great','thing','love','like']);
