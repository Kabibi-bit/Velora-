...
function renderNav(activePage){
  ...
  const logoHrefMap = { candidate: 'overview.html', tutor: 'tutor-overview.html' };
...
function renderWaypointJournal(opts){
  ...
  if(entries.length === 0){
    listEl.innerHTML = `<div class="empty-state"><div class="glyph">&#128269;</div>No entries match "${opts.searchQuery}".<div class="sub">Try a different word, or clear the search.</div></div>`;
    const patternPanel = document.getElementById(opts.patternPanelId);
    if(patternPanel) patternPanel.classList.add('hidden');
    return;
  }
...
/* ---- Generic tag/keyword-overlap scoring, reusable for athletes matching listings and tutors matching requests ---- */
