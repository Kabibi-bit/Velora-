...
function computeFactorReliability(applicationsWithOutcomes){
  ...
  FACTOR_NAMES_JS.forEach(factor => {
    const engaged = usable.filter(a => (a.factors_snapshot[factor] || 0) > 0);
    const n = engaged.length;
    if(n < 2){ multipliers[factor] = 1.0; return; }
    const engagedPositive = engaged.filter(a => POSITIVE_STATUSES_JS.has(a.outcome_status)).length;
    const engagedRate = engagedPositive / n;
...
function getPersonalizedFactorWeightsJS(){
  ...
  return computeFactorReliability(apps.map(a => ({ factors_snapshot: a.factors_snapshot, outcome_status: a.outcome_status })));
}
...
function scoreListing(listing, goalTokens, skillTokens, profile, factorWeights){
  ...
  tagSet.forEach(tag=>{
    ...
    if(inGoal){ goalFit += 3; matchedGoal.push(tag); }
    if(inSkill){ skillFit += 2; matchedSkill.push(tag); }
  ...
  const score = goalFit + skillFit + priorityFit + locationValue + deadlineValue;
  const denom = tagSet.length*3 + 2 + 1.5;
  const pct = Math.max(35, Math.min(97, Math.round((score / denom) * 100)));

  const factorsEngaged = [goalFit, skillFit, priorityFit, locationValue, deadlineValue].filter(v => v > 0).length;
  ...
  return {
    ...
    factors: { goalFit, skillFit, priorityFit, locationFit: locationValue, locationReason: location.reason, deadlineUrgency: deadlineValue, daysLeft: deadline.daysLeft },
  };
}
