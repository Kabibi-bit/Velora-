...
function _matchFromBackend(m){
  return {
    ...
    // Signal Score (fit + freshness + ghost-risk) — Velora's honest reframing
...
async function apiResearchProgram(sport, level, programName){
  const result = await apiFetch('/athletics/research-program', {
    method: 'POST',
    body: { sport: sport || '', level: level || '', program_name: programName || '' },
  });
  if(!result.ok) return null;
  return result.data || null;
}
...
// Analyzes a game video by URL and returns candidate action moments + timestamps.
// The CV inference runs on the backend's Roboflow integration; this just calls it.
// Returns { ok, moments, note, analyzed_frames } - or a safe shape on failure.
async function apiDetectHighlights(userId, videoUrl){
  const result = await apiFetch('/athlete/detect-highlights', {
    method: 'POST',
    body: { user_id: userId, video_url: videoUrl }
  });
  if(result.ok && result.data) return result.data;
  return { ok: false, moments: [], note: "Couldn't run highlight detection just now. You can still log your own moments below." };
}
...
async function apiDetectHighlights(userId, videoUrl, sport){
  ...
  if(!result.ok) return { available:false, moments:[], note:'Could not reach the detection service right now.' };
  return result.data;
}
