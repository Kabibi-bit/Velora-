...
/* ============================================================================
   SIGNAL SCORE — Velora's answer to the discredited "match %".
...
function runMatchCycle(profile){
  ...
  let scored = candidates.map(l=>{
    ...
    const scored = {...l, ...m};
    // Attach the Signal Score (fit + freshness + ghost-risk) - Velora's honest
...
function scoreByOverlap(itemTags, requirementText){
  ...
  // inflation Velora rejects. Now: no tags or no overlap -> a genuinely low score
