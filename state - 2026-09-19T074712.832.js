...
function setTier(tier){
  ...
  localStorage.setItem('velora_tier', tier);
  return true;
}
...
function saveAthleteOutcomes(map){ localStorage.setItem('velora_athlete_outcomes', JSON.stringify(map || {})); }
...
function _saveAdmissionsMilestones(arr){
  localStorage.setItem(ADMISSIONS_MILESTONES_KEY, JSON.stringify(arr.slice(-60)));
}
...
function saveUserPreferenceWeightsJS(weights){
  ...
  try{ localStorage.setItem('velora_pref_weights', JSON.stringify(clean)); }catch(e){}
  return clean;
}
...
function scoreCareerDirections(answers){
  ...
  const _dim = v => { const x = Number(v); return Number.isFinite(x) ? x : 1; };  // default 1, matching the backend's answers.get(k, 1)
