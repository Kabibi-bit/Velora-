...
function athleteRationale(listing, matched, profile){
  profile = profile || {};
  matched = matched || [];
  const div = (profile.targetDivision || '').trim();
  const pos = (profile.position || '').trim();
  const divBit = div ? ` (targeting ${div})` : '';
  if(matched.length === 0){
    return `Looser fit for ${profile.sport || 'this sport'} at the ${profile.level || 'this'} level${divBit} - no strong overlap yet, but worth a look while broadening your search.`;
  }
  const posBit = pos ? `, relevant to your position (${pos})` : '';
  return `Matches on <b>${matched.slice(0,3).join(', ')}</b> - directly relevant to ${profile.sport || 'your sport'}${posBit} and your stated direction toward ${(profile.careerDirection || '').replace(/-/g, ' ')}${divBit}.`;
}
